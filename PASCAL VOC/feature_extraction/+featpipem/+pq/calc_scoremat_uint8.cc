...
void calcScoreMat(const mwSize n, const mwSize class_count, const mwSize pq_dim,
		  const mxArray* LUT, const uintType* pqcodes,
		  float** scoremat) {
  float feat_score;
  mxArray* LUTcell;
  float* LUTarr;
...
void mexFunction( int nlhs, mxArray *plhs[], 
  ...
  mxUINT8_CLASS:
    ...
    calcScoreMat<uint8_t>(n, class_count, pq_dim, prhs[0], pqcodes, &scoremat);
    break;
  mxUINT16_CLASS:
    uint16_t* pqcodes = (uint16_t*)mxGetData(prhs[2]);
    calcScoreMat<uint16_t>(n, class_count, pq_dim, prhs[0], pqcodes, &scoremat);
    break;
  mxUINT32_CLASS:
    uint32_t* pqcodes = (uint32_t*)mxGetData(prhs[2]);
    calcScoreMat<uint32_t>(n, class_count, pq_dim, prhs[0], pqcodes, &scoremat);
    break;
  default:
